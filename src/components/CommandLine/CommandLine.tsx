...
import { setTheme, setCommandLine } from "../../slice/app";
import { addFavoriteTheme, removeFavoriteTheme } from "../../slice/config";
...
function CommandLine() {
  ...
  const deleteCommand = async () => {
    ...
    // dispatch(
    //   setTheme((await import(`../../themes/${config.themeName}.ts`)).default)
    // );
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const isCustom = Boolean(configItems.find(( [ key ] ) => key === selected)?.[1]
      .custom);
  ...
  useLayoutEffect(() => {
    ...
    if (selected) {
      const selectedOptions = configList[selected].options
        ...
        .sort(
          (a, b) => a.toString().search(regex) - b.toString().search(regex)
        );

      const selectedindex = selectedOptions.reduce(
        (index: number, option, currentIndex) => {
      ...
      const configItems = Object.entries(configList)
        ...
        .sort(
          ([, a], [, b]) => a.command.search(regex) - b.command.search(regex)
        );

      if (value.endsWith(" ")) {
        const item = configItems.find(
          ([, { command }]) => command === trimmedValue
        );
  ...
  useEffect(() => {
    (list.current?.children[activeIndex] as HTMLDivElement)?.scrollIntoView({
      block: "nearest",
    });
  }, [activeIndex]);
  ...
  useEffect(() => {
    if (selected === "themeName" && !isPresent) {
      (async () => { 
        ...
        // dispatch(
        //   setTheme(
        //     (await import(`../../themes/${config.themeName}.ts`)).default
        //   )
        // );
      })();
    }
  ...
  return (
    //  has to component which  is content and wrapper the context 
    <Popup
      top
      maxWidth={700}
          ...
          configItems.map(([key, { command }], index) => (
            ...
            >
              ...
              {(configItems.length === 1 ||
                filterValue(inputValue) === command) && (
